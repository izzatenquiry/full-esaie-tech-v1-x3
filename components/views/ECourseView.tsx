...
const PlatformUpdatesView: React.FC<PlatformUpdatesViewProps> = ({ language }) => {
    ...
    const getStatusIndicator = (systemStatus: PlatformSystemStatus) => {
        switch (systemStatus) {
            case 'operational':
                return { Icon: CheckCircleIcon, color: 'text-green-500', text: T.status.operational };
            case 'degraded':
                return { Icon: AlertTriangleIcon, color: 'text-yellow-500', text: T.status.degraded };
            case 'outage':
                return { Icon: XIcon, color: 'text-red-500', text: T.status.outage };
        }
    };
...
const ECourseView: React.FC<ECourseViewProps> = ({ currentUser, language }) => {
    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm">
            <h2 className="text-xl font-bold mb-4">{T.getStarted}</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {content.tutorials.map((tutorial, index) => (
                      <div key={index} className="bg-white dark:bg-neutral-900 rounded-lg p-5 flex flex-col shadow-sm">
                        ...
                        <a href="#" className="text-primary-600 dark:text-primary-400 font-semibold mt-4 text-sm hover:underline">{T.learnMore}</a>
                      </div>
                    ))}
                  </div>
              </div>
