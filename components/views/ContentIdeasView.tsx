...
import { type GenerateContentResponse } from '@google/ai';
...
const ContentIdeasView: React.FC<ContentIdeasViewProps> = ({ language }) => {
    ...
    const handleGenerate = useCallback(async () => {
        if (!topic.trim()) {
            // FIX: Property 'errorNoTopic' does not exist on type '...'. Added it to translations.ts
    ...
    }, [topic, selectedLanguage, T.errorNoTopic]);
        ...
        <>
             {response && !isLoading && (
                <div className="absolute top-3 right-3 flex gap-2 z-10">
                    ...
                    <button
                        onClick={() => downloadText(response.text ?? '', `esaie-tech-content-ideas-${Date.now()}.txt`)}
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                    <Spinner />
                    {/* FIX: Property 'generating' does not exist on type '...'. Added it to translations.ts */}
                    <p className="text-neutral-500 dark:text-neutral-400">{T.generating}</p>
                </div>
            ) : response ? (
