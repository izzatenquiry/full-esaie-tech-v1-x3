...
const getAiAgents = (language: Language): AiAgent[] => {
    const T = getTranslations(language).staffMonoklixView.agents;
    const icons: { [key: string]: React.ComponentType<{ className?: string }> } = {
        wan: UserIcon, tina: SmileyIcon, jamil: LightbulbIcon, najwa: FileTextIcon,
        saifuz: ClipboardListIcon, mieya: TrendingUpIcon, afiq: StoreIcon, julia: MegaphoneIcon,
        mazrul: FilmIcon, musa: UsersIcon, joe_davinci: ImageIcon, zaki: GalleryIcon
    };
    return T.map(agent => ({ ...agent, icon: icons[agent.id] }));
};
...
const StaffMonoklixView: React.FC<StaffMonoklixViewProps> = ({ language }) => {
    ...
    const aiAgents = useMemo(() => getAiAgents(language), [language]);
    ...
    const selectedAgent = useMemo(() => aiAgents.find(agent => agent.id === selectedAgentId)!, [selectedAgentId, aiAgents]);
    ...
    const handleGenerate = useCallback(async () => {
        ...
        try {
            ...
            await addHistoryItem({
                type: 'Copy',
                prompt: `Staff ESAIE (${selectedAgent.name}): ${userInput.substring(0, 50)}... (Lang: ${selectedLanguage})`,
                result: result,
            });
        } catch (e) {
        ...
        <>
             {generatedCopy && !isLoading && (
                <div className="absolute top-3 right-3 flex gap-2 z-10">
                    ...
                    <button
                        onClick={() => downloadText(generatedCopy, `esaie-tech-staff-${selectedAgent.id}.txt`)}
             ...
             {isLoading ? (
                <div className="flex flex-col items-center justify-center h-full gap-4">
                    <Spinner />
                    <p className="text-neutral-500 dark:text-neutral-400">{commonT.generating}</p>
                </div>
            ) : generatedCopy ? (
