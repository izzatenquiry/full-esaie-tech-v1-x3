...
const CaseCard: React.FC<CaseCardProps> = ({ caseItem, onUsePrompt, language }) => {
    const [copied, setCopied] = useState(false);
    const T = getTranslations(language).libraryView;
    ...
    return (
        <div className="bg-white dark:bg-neutral-900 p-5 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-800 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 border border-neutral-300 dark:border-neutral-700 rounded-md overflow-hidden">
                <div className="p-1">
                    <p className="font-semibold text-sm text-center mb-1">{T.input}</p>
                    {caseItem.inputImageUrl ? (
                        <img src={caseItem.inputImageUrl} alt="Input" className="w-full aspect-square object-cover rounded-sm"/>
                    ) : (
                        <div className="w-full aspect-square bg-neutral-100 dark:bg-neutral-800 rounded-sm flex items-center justify-center text-xs text-neutral-400">{T.noInput}</div>
                    )}
                </div>
                <div className="p-1">
                     <p className="font-semibold text-sm text-center mb-1">{T.output}</p>
                    {caseItem.outputImageUrl ? (
                        <img src={caseItem.outputImageUrl} alt="Output" className="w-full aspect-square object-cover rounded-sm"/>
                    ) : (
                        <div className="w-full aspect-square bg-neutral-100 dark:bg-neutral-800 rounded-sm flex items-center justify-center text-xs text-neutral-400">{T.noOutput}</div>
                    )}
                </div>
            </div>

            <div>
                <p className="text-sm font-semibold mb-1">{T.input.toLowerCase()}:</p>
...
const LibraryView: React.FC<LibraryViewProps> = ({ onUsePrompt, language }) => {
                ...
                filteredCases.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                        {filteredCases.map((caseItem) => (
                            <CaseCard key={caseItem.title} caseItem={caseItem} onUsePrompt={onUsePrompt} language={language} />
                        ))}
                    </div>
                ) : (
                    <div className="text-center py-20 text-neutral-500 dark:text-neutral-400">
                        ...
                        <p className="text-sm">{searchTerm ? T.noResultsTerm.replace('{term}', searchTerm) : T.empty}</p>
                    </div>
                )
            )}
        </div>
