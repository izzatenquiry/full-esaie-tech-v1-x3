...
const BatchProcessorView: React.FC<BatchProcessorViewProps> = ({ preset, clearPreset, language }) => {
  ...
  const [logs, setLogs] = useState<Log[]>([{ timestamp: new Date().toLocaleTimeString(), message: T.engineReady, type: 'info' }]);
  ...
  useEffect(() => {
    if (preset) {
        ...
        addLog(T.loadedFromStoryboard.replace('{count}', String(formattedPreset.length)), 'info');
        clearPreset();
    }
  }, [preset, clearPreset, addLog, T.loadedFromStoryboard]);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
        ...
        const finalChunks = chunks.filter(chunk => {
            ...
            return (chunk.startsWith('**Scene') || chunk.startsWith('**Babak')) && !lowerChunk.includes('storyboard');
        });

        let items: BatchItem[];
        if (finalChunks.length > 0) {
            items = finalChunks.map(chunk => ({ prompt: chunk.trim() }));
        } else {
        ...
        addLog(T.loadedFromFile.replace('{count}', String(items.length)).replace('{fileName}', file.name), 'info');
    };
  ...
  const handleStartProcess = async () => {
    ...
    addLog(T.starting.replace('{count}', String(batchItems.length)), 'info');

    for (let i = 0; i < batchItems.length; i++) {
        if (isCancelledRef.current) {
            addLog(T.cancelled, 'info');
            break;
        }

        const item = batchItems[i];
        const prompt = item.prompt;
        addLog(T.processing.replace('{current}', String(i + 1)).replace('{total}', String(batchItems.length)).replace('{prompt}', prompt), 'info');
        ...
        try {
            ...
            addLog(T.success.replace('{prompt}', prompt), 'success');
            setProgress(p => ({ ...p, completed: p.completed + 1 }));
        } catch (e) {
            const errorMessage = e instanceof Error ? e.message : "Unknown error";
            addLog(T.fail.replace('{prompt}', prompt).replace('{error}', errorMessage), 'error');
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
      <div className="bg-white dark:bg-neutral-900 p-6 rounded-lg shadow-sm flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div>
            <label htmlFor="file-upload" className="flex flex-col items-center justify-center border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors duration-300 h-32 border-neutral-300 dark:border-neutral-700 hover:border-primary-400 hover:bg-neutral-100 dark:hover:bg-neutral-800/50">
                ...
                <p className="text-sm text-neutral-600 dark:text-neutral-400">{T.upload}</p>
            ...
            {fileName && (
                <div className="mt-2 text-center text-sm text-green-600 dark:text-green-400 flex items-center justify-center gap-2">
                    ...
                    <span>{T.loadedFromFile.replace('{count}', String(batchItems.length)).replace('{fileName}', fileName)}</span>
                </div>
            )}
        </div>
        ...
        <div className="flex-1 bg-neutral-100 dark:bg-neutral-800/50 rounded-md p-3 space-y-3 overflow-y-auto min-h-[200px] custom-scrollbar">
            {batchItems.length > 0 ? (
                ...
                <div className="text-center text-xs text-neutral-500 h-full flex items-center justify-center">{T.uploadPlaceholder}</div>
            )}
        </div>
        {batchItems.length > 0 && <button onClick={clearItems} disabled={isProcessing} className="text-sm text-red-500 hover:underline disabled:opacity-50">{T.clearAll}</button>}
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-lg flex flex-col p-6 shadow-sm">
        <h3 className="text-xl font-bold mb-4 flex-shrink-0">{T.logAndProgress}</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4 flex-shrink-0">
            <div className="bg-neutral-100 dark:bg-neutral-800/50 p-4 rounded-lg text-center">
                <p className="text-sm text-neutral-500 dark:text-neutral-400">{T.prompts}</p>
                <p className="text-2xl font-bold">{batchItems.length}</p>
            </div>
            <div className="bg-green-100 dark:bg-green-900/30 p-4 rounded-lg text-center">
                <p className="text-sm text-green-800 dark:text-green-300">{T.generated}</p>
            ...
            <div className="bg-red-100 dark:bg-red-900/30 p-4 rounded-lg text-center">
                <p className="text-sm text-red-800 dark:text-red-300">{T.failed}</p>
        ...
        <div className="flex-1 flex flex-col bg-neutral-100 dark:bg-neutral-950 rounded-lg p-2 min-h-0">
            <h4 className="text-lg font-semibold mb-2 px-2 flex-shrink-0">{T.activityLog}</h4>
