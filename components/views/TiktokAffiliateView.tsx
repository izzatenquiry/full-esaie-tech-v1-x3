...
const TiktokAffiliateView: React.FC<TiktokAffiliateViewProps> = ({ onReEdit, onCreateVideo, language, currentUser, onUserUpdate }) => {
    ...
    const modelFaceOptions = T.modelFaceOptions;
    const lightingOptions = T.lightingOptions;
    const cameraOptions = T.cameraOptions;
    const poseOptions = T.poseOptions;
    const vibeOptions = T.vibeOptions;
    ...
    const handleGenerate = useCallback(async () => {
        ...
        setIsLoading(false);
        setProgress(0);
    }, [numberOfImages, productImage, generateOneImage, T.errorNoImage]);
    ...
    const ActionButtons: React.FC<{ imageBase64: string; mimeType: string }> = ({ imageBase64, mimeType }) => (
        <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            <button onClick={() => triggerDownload(imageBase64, 'esaie-tech-model-photo')} title="Download Image" className="flex items-center justify-center w-8 h-8 bg-black/60 text-white rounded-full hover:bg-black/80 transition-colors"><DownloadIcon className="w-4 h-4" /></button>
        </div>
    );
