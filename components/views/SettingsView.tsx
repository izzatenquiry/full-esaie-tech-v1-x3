...
type SettingsTabId = 'profile' | 'api' | 'user-db';

const getTabs = (T: any): Tab<SettingsTabId>[] => [
...
const ProfilePanel: React.FC<Pick<SettingsViewProps, 'currentUser' | 'onUserUpdate' | 'language'>> = ({ currentUser, onUserUpdate, language }) => {
    const T = getTranslations(language).settingsView.profile;
...
const CacheManagerPanel: React.FC<{ language: Language }> = ({ language }) => {
...
const ApiIntegrationsPanel: React.FC<ApiIntegrationsPanelProps> = ({ currentUser, onUserUpdate, language, veoTokenRefreshedAt, assignTokenProcess }) => {
    const T = getTranslations(language).settingsView.api;
                ...
                <div className="border-t border-neutral-200 dark:border-neutral-800 pt-8">
                    ...
                    <div className="flex items-center gap-2 mt-4">
                        <button onClick={handleSaveWebhook} disabled={webhookStatus.type === 'loading'} className="bg-primary-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-primary-700 w-24 flex justify-center">
                            {webhookStatus.type === 'loading' && webhookStatus.message.includes(T.savingWebhook) ? <Spinner /> : T.saveWebhook}
                        </button>
                        <button onClick={handleTestWebhook} disabled={!currentUser.webhookUrl || webhookStatus.type === 'loading'} className="bg-neutral-200 dark:bg-neutral-700 font-semibold py-2 px-4 rounded-lg hover:bg-neutral-300 disabled:opacity-50 w-40 flex justify-center">
                            {webhookStatus.type === 'loading' && webhookStatus.message.includes(T.testingWebhook) ? <Spinner /> : T.testWebhook}
                        </button>
                    </div>
...
const SettingsView: React.FC<SettingsViewProps> = (props) => {
    const T = getTranslations(props.language).settingsView;
    ...
    const renderActiveTabContent = () => {
        switch (activeTab) {
            case 'profile': 
                return (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                        <ProfilePanel currentUser={currentUser} onUserUpdate={props.onUserUpdate} language={language} />
                        <CacheManagerPanel language={language} />
                    </div>
                );
            case 'api': 
            ...
            default: return <ProfilePanel currentUser={currentUser} onUserUpdate={props.onUserUpdate} language={language} />;
        }
    };
