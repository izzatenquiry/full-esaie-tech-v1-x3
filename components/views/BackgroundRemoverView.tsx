...
const BackgroundRemoverView: React.FC<BackgroundRemoverViewProps> = ({ onReEdit, onCreateVideo, currentUser, onUserUpdate, language }) => {
  ...
  }, [imageData, currentUser, onUserUpdate, T.errorNoImage]);
  ...
  const leftPanel = (
    <>
      ...
      <div className="flex-1 flex flex-col justify-center">
          ...
          <ImageUpload key={imageUploadKey} id="bg-remover-upload" onImageUpload={handleImageUpload} onRemove={handleRemoveImage} title={T.upload} language={language}/>
      </div>
                    ...
                    <div>
                        ...
                        <div className="relative group bg-gray-200 dark:bg-gray-700 rounded-lg" style={{backgroundImage: 'repeating-conic-gradient(#e5e7eb 0 25%, transparent 0 50%)', backgroundSize: '16px 16px'}}>
                            ...
                            <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                               ...
                               <button onClick={() => triggerDownload(resultImage, 'esaie-tech-bg-removed')} title="Download Image" className="flex items-center justify-center w-8 h-8 bg-black/60 text-white rounded-full hover:bg-black/80 transition-colors"><DownloadIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                </div>
