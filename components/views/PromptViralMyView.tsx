...
const ViralPromptCard: React.FC<ViralPromptCardProps> = ({ promptItem, onUsePrompt, language }) => {
        ...
        <div className="bg-white dark:bg-neutral-900 p-5 rounded-lg shadow-sm border border-neutral-200 dark:border-neutral-800 flex flex-col gap-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400">{T.by} {promptItem.author}</p>
              </div>
            </div>
            ...
            <div>
                <p className="text-sm font-semibold mb-1">{T.prompt}:</p>
                ...
                filteredPrompts.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                        {filteredPrompts.map((promptItem) => (
                            <ViralPromptCard key={promptItem.id} promptItem={promptItem} onUsePrompt={onUsePrompt} language={language} />
                        ))}
                    </div>
                ) : (
