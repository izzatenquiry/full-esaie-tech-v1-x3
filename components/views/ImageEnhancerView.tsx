...
const ImageEnhancerView: React.FC<ImageEnhancerViewProps> = ({ onReEdit, onCreateVideo, currentUser, onUserUpdate, language }) => {
  ...
  const handleEnhance = useCallback(async () => {
    ...
    try {
      ...
      if (imageBase64) {
        ...
        setError("AI could not enhance the image. Please try a different image.");
      }
    } catch (e) {
  ...
  }, [imageData, enhancementType, currentUser, onUserUpdate, T.errorNoImage]);
  ...
  const leftPanel = (
    <>
      ...
      <div className="flex-1 flex flex-col justify-center">
          ...
          <ImageUpload key={imageUploadKey} id="enhancer-upload" onImageUpload={handleImageUpload} onRemove={handleRemoveImage} title={T.upload} language={language}/>
      </div>
      
      <div className="space-y-4 pt-4 mt-auto">
          <div className="flex justify-center gap-4">
              <button onClick={() => setEnhancementType('upscale')} className={`px-6 py-2 rounded-full font-semibold transition-colors text-sm ${enhancementType === 'upscale' ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>{T.upscale}</button>
              <button onClick={() => setEnhancementType('colors')} className={`px-6 py-2 rounded-full font-semibold transition-colors text-sm ${enhancementType === 'colors' ? 'bg-primary-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600'}`}>{T.colors}</button>
          </div>
                    ...
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                               ...
                               <button onClick={() => triggerDownload(resultImage, 'esaie-tech-enhanced')} title="Download Image" className="flex items-center justify-center w-8 h-8 bg-black/60 text-white rounded-full hover:bg-black/80 transition-colors"><DownloadIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                </div>
