...
const ApiHealthCheckModal: React.FC<ApiHealthCheckModalProps> = ({ isOpen, onClose, user, language }) => {
    ...
    const [activeKeyForDisplay, setActiveKeyForDisplay] = useState<string | null | undefined>(null);
    ...
    const handleHealthCheck = async () => {
        ...
        // The key for Gemini text models is always the shared session key
        const textKey = sessionStorage.getItem('esaie_tech_session_api_key');
        
        // The auth token for Veo/Imagen is the user's personal token if checking a specific user
        const authToken = user?.personalAuthToken;

        // For display, we prioritize showing the user's specific token.
        setActiveKeyForDisplay(authToken || textKey);
        
        try {
            const checkResults = await runApiHealthCheck({
                textKey: textKey || undefined,
                    ...
                    {isChecking ? (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center p-2 bg-neutral-100 dark:bg-neutral-800 rounded-md text-sm">
                                <span className="font-semibold text-neutral-600 dark:text-neutral-300">{user ? "User Auth Token:" : T.activeKey}</span>
                                {activeKeyForDisplay ? (
                                    <span className="font-mono text-green-600 dark:text-green-400">...{activeKeyForDisplay.slice(-4)}</span>
                                ) : (
