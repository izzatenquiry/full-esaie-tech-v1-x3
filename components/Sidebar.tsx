...
import {
  ImageIcon, VideoIcon, SettingsIcon, BookOpenIcon, LogoutIcon, GalleryIcon, XIcon, LibraryIcon, FileTextIcon, GraduationCapIcon, WhatsAppIcon
} from './Icons';
...
const getNavItems = (language: Language): NavItem[] => {
    ...
    return [
      ...
      { id: 'support-group', label: T.supportGroup, section: 'bottom', icon: WhatsAppIcon, isExternal: true, url: 'https://chat.whatsapp.com/By54h2HP4p78EuBA8XRpLn' },
  ...
  const renderNavItem = (item: NavItem) => {
    ...
    const content = (
      <>
        ...
        {item.isNew && !isDisabled && <span className="text-xs bg-primary-500/20 text-primary-500 dark:text-primary-400 font-bold px-2 py-0.5 rounded-full">{T.new}</span>}
      </>
    );
