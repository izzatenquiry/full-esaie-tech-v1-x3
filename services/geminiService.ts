...
export const generateVoiceOver = async (
    ...
    try {
        ...
        if (generationMode === 'sing') {
            ...
            if (language === 'Malay') {
                singInstruction = `Sing the following lyrics in a ${musicStyle || 'pop'} music style in Malay`;
            }
            ...
            if (language === 'Malay') {
                languageInstruction = 'Say the following in clear Malay: ';
            }
...
export const runApiHealthCheck = async (keys: { textKey?: string; authToken?: string }): Promise<HealthCheckResult[]> => {
    console.log('--- Starting Full API Health Check ---');
    const { textKey, authToken } = keys;

    const ai = textKey ? new GoogleGenAI({ apiKey: textKey }) : null;
    ...
    if (ai) {
        ...
        results.push({ service: 'Text Generation', model: MODELS.text, status: 'degraded', message: 'Health check skipped. Gemini API Key not found.' });
        console.warn('   ⚠️ Text Generation check skipped: No Gemini API Key found.');
    }
    
    // Get tokens for proxied services
    const getSessionTokens = (): { token: string; createdAt: string }[] => {
    ...
    // If a specific authToken is provided for a user check, use it. Otherwise, use session tokens for a general check.
    const proxyTokens = authToken ? [{ token: authToken, createdAt: new Date().toISOString() }] : getSessionTokens();
        ...
        for (let i = 0; i < proxyTokens.length; i++) {
            ...
            try {
                ...
                results.push({ 
                    ...
                    details: authToken ? `(Using user's personal token)` : `(Using token #${i + 1})`
                });
